...
let date = $computed(() => posts[findCurrentIndex()]?.date)
let nextPost = $computed(() => posts[findCurrentIndex() - 1])
let prevPost = $computed(() => posts[findCurrentIndex() + 1])
</script>

<template>
  <Layout>
    <router-view v-slot="{ Component: Post }">
